...
const getData = () => {
    ...
    fetch(`https://v6.exchangerate-api.com/v6/41b4541f3df8b629ff6e1018/latest/${currencyOne}`)
        ...
        .then((data) => {
            ...
            rateEl.textContent = `1${currencyOneEl.value} = ${currencyFromBack[currencyTwoEl.value]}${currencyTwoEl.value}`
...
swapEl.onclick = () => {
    const reverse = currencyOneEl.value;
    currencyOneEl.value = currencyTwoEl.value;
    currencyTwoEl.value = reverse;
    getData();
    calculateRates();
}
